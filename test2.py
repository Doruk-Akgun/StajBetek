...
# LM Studio bağlantı adresiniz (Bunu kendi ekranınızdaki URL ile güncelleyin)
...
def get_embedding(text):
    ...
    if response.status_code != 200:
        raise Exception(f"LM Studio Hatası: {response.status_code} - {response.text}")
    
    return response.json()['data'][0]['embedding']   # <-- [0] eklendi
